 ...
 *    transfers min(n, cnt) bytes from an internal buffer to a user
 *    buffer, where n is the number of bytes requested by the user and
 *    cnt is the number of unread bytes in the internal buffer. On
...
void parse_request(int fd, http_request *req)
{
    ...
    url_decode(name, req->function_name, MAXLINE);
}
    ...
    while (*p && (*p) != '/' && (*p) != '\0') {
        ...
        if (*p == '/') {
            *p = ' ';
            ...
            /* Prevent source path from being edit */
            /* FIXME: if command is time/source/..., it will error */
            if (!strncmp(req.function_name, "source", 6)) {
                break;
            }
        }
    }
